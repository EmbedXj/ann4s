...
// fork from https://github.com/pishen/annoy4s
...
struct FixRandom {
  ...
  inline int flip() {
    int r = fix & 1;
    fix ++;
    std::cerr << "n-flip: " << r << std::endl;
    return r;
  }
  inline size_t index(size_t n) {
    size_t r = fix % n;
    fix ++;
    std::cerr << "n-index: " << r << " " <<  n << std::endl;
    return r;
  }
};
...
AnnoyIndexInterface<int32_t, float> *createAngular(int f) {
  return new AnnoyIndex<int32_t, float, Angular, FixRandom>(f);
}

AnnoyIndexInterface<int32_t, float> *createEuclidean(int f) {
  return new AnnoyIndex<int32_t, float, Euclidean, FixRandom>(f);
}
